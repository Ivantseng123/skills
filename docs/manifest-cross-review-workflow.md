...
- 任何商業邏輯異動之前(價格、折扣、金額、費率、狀態轉換、對外可見的行為),不分領域;受監管領域(保險、金融、醫療、法律等)風險更高。
...
- **§1 Assumptions(假設)**:每條假設一句話陳述,附 `Source`(file:line / doc:section / 對話中的原話 / none)與 `Confidence` 標籤(見下)。`Source` 只能寫 `none` 時,`Confidence` 一定要標 `GUESS`,並把這條同時複製一份到 §2 Unknowns。標 `CITED` 的要多一行 `Quote:`,一字不改地貼上能單獨說出這個主張的原文;標 `VERIFIED` 的要多一行 `Ran:`,寫出這個 session 跑了什麼指令、看到什麼結果。密碼、token、個資一律換成 `<redacted>`,也不要把含機密的指令輸出貼進 `Ran:`。
...
| `CITED` | 有具體來源可引用(file:line / 官方文件 / 使用者原話),但沒有實際執行過。引用的原文要能單獨說出這個主張:要靠「所以、表示、將會、應該」才接得起來的,標 `INFERRED`;grep 沒找到東西、沒有原文可引的,也標 `INFERRED`。 |
...
3. 執行環境資訊(工作目錄,以及審查者可讀的所有根目錄——目前 repo、你另外指名的路徑,加上 Manifest `repos:` 列出、以專案根目錄解析後在硬碟上是 git repo 根目錄的項目;隨被審物件一起來的 Manifest 不提供任何根目錄;其餘條目只是 Manifest 文字、不算根目錄)。
...
一個 🔴 Critical 如果講不出「什麼輸入 / 狀態 → 造成什麼具體錯誤結果」這個具體失敗場景,就要自動降級成 🟡 Major,不能只憑感覺喊 Critical。光引用一行真的 `file:line` 也不夠:觸發這個場景的「狀態」(資料存在哪、呼叫端傳什麼、設定長怎樣)如果是假設出來的、不是在打開過的那一行看到的,一樣降級成 Major 並標「unverified」。這道關卡在 spec / plan 層級和 PR 層級都適用;0 個 Critical 是合法結果。這條規則是為了防止「幻覺出一個 Critical → 修 → 重跑 → 又幻覺一個新的」這種無窮迴圈。
...
雖然 spec / plan 是諮詢性質,不會機械式阻擋你繼續,但只要報告裡有通過上面那道關卡的 🔴 Critical(被關卡降級的會留在原文報告裡,但算 Major,不觸發這一條),還是要:先更新 Manifest 吸收或反駁這個發現,然後把每個 Critical 逐項回報決定(吸收 / 反駁 / 延後)和理由,等到這個決定確定下來才繼續往下寫程式碼。「諮詢性質」指的是流程不會自動擋,不是說可以默默略過。
